...
class GrpcCredentialsRefreshError(OSError):
    ...
    the user must re-authenticate) from a transient/server error (retry).
    """
...
def keys_need_refresh(
    ...
    *,
    ...
    """True when the session keys or OAuth access token are missing/near expiry."""
    now = int(time.time())
...
async def async_refresh_access_token(
    ...
    async with session.post(
        ...
        try:
            ...
            msg = f"Sony Seeds token request failed: HTTP {exc.status}"
            raise GrpcCredentialsRefreshError(msg, status=exc.status) from exc
        return await response.json()
...
def device_hardware_info(device: dict[str, Any]) -> dict[str, str | None]:
    """Extract HA device attributes from a Sony IoT ``/devices`` entry.

    The payload nests the model under ``device_infos`` and the consumer SKU
    under ``attributes.identified_model_name``.
    """
